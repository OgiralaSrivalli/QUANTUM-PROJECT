...
def predict_job_completion(user_name: str, job_id: str):
    """Predict when a job will likely complete - Frontend Feature: Time Predictor"""
    user = next((u for u in USERS if u["name"].lower() == user_name.lower()), None)
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    try:
        ...
        queue_info = job_data.get("queue_info", {})
        est_start = queue_info.get("estimated_start_time")
        exec_time = job_data["usage"].get("seconds", 0)

        # -------------------------
        # Fallback: Historical average if exec_time missing
        # -------------------------
        if not exec_time or exec_time == 0:
            past_jobs = service.jobs(limit=20)
            times = []
            for past_job in past_jobs:
                past_data = extract_job_data(past_job)
                if past_data["usage"].get("seconds"):
                    times.append(past_data["usage"]["seconds"])
            if times:
        ...
        if est_start and est_start != "None":
            try:
                predicted_start = datetime.fromisoformat(est_start.replace("Z", "+00:00"))
            except:
                predicted_start = None

        if predicted_start and exec_time:
            predicted_end = predicted_start + timedelta(seconds=exec_time)
        elif exec_time:
            predicted_end = datetime.now() + timedelta(seconds=exec_time)
        ...
        return {
            ...
            "backend": job_data["backend"],
            "status": job_data["status"],
            "predicted_start": predicted_start.isoformat() if predicted_start else "Unknown",
            "predicted_end": predicted_end.isoformat() if predicted_end else "Unknown",
            "average_execution_seconds": round(exec_time, 2) if exec_time else "Unknown"
        }
